...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "mmdetection")
...
def build_dataset(cfg, ann, mode):
    ...
    if mode in ("train", "valid"):
        ...
        pipeline = pipeline(
            resize_to=cfg["transform"]["resize_to"],
            mode=mode,
            concat=cfg_concat,
            **cfg["transform"]["preprocess"]
        )
    else:
        if "augmentations" in cfg["transform"]["preprocess"]:
            _ = cfg["transform"]["preprocess"].pop("augmentations")
        pipeline = getattr(transforms, cfg["transform"]["augment"]["infer"])
        pipeline = pipeline(
            resize_to=cfg["transform"]["resize_to"], **cfg["transform"]["preprocess"]
        )
    ...
    dset_params = (
        cfg["dataset"]["params"] if cfg["dataset"]["params"] is not None else {}
    )

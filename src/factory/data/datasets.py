...
class XrayDataset(Dataset):
    ...
    def __init__(self,
        ...
        if not test_mode and filter_empty:
            self.logger.info('Found {:,} annotations with bounding boxes ...'.format(len(self.img_infos)))
            if not self.positives_only:
                self.logger.info('Found {:,} annotations without bounding boxes ...'.format(len(self.neg_infos)))
    ...
    def prepare_train_img(self, idx):
        ...
        if not self.positives_only:
            # Add negative image
            img_info['negative'] = np.random.choice(self.neg_infos)['filename']
